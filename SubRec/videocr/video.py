...
class Video:
    ...
    def __init__(self, path: str):
        ...
        with Capture(path) as v:
            self.num_frames = int(float(v.get(cv2.CAP_PROP_FRAME_COUNT)))
            self.fps = v.get(cv2.CAP_PROP_FPS)
            self.height = int(float(v.get(cv2.CAP_PROP_FRAME_HEIGHT)))
    ...
    def _generate_subtitles(self, sim_threshold: int) -> None:
        ...
        WIN_BOUND = int(float(self.fps // 2))  # 1/2 sec sliding window boundary
        bound = WIN_BOUND
        i = 0
        j = 1
